#1 If translation is present on purpleculture don't use google_trans_new (slow/wrong)
#2 Exit fullscreen on f11
#3 Add the possibility to go back to the previous character
...
while not done:
    ...
    # The Amount of the New Words Left
    font = pygame.font.Font('fonts\Gentium-R.ttf', 25)
    label = font.render('New: {}'.format(len(words)), True, GREY)
    position = label.get_rect()
    position.center = (69, 100)
    screen.blit(label, position)
