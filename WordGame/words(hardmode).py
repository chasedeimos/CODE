...
time.sleep(1)
print('>{}{}:4)Одолевший меня получит мой личный пароль.'.format(botname,emotion['rapture']))
time.sleep(2)
face = emotion['cute']
...
print('\n')
                                #>>>Main Loop
while game:
    word_raw = input('>{}:'.format(username))
    word = word_raw[0].upper()+word_raw[1:].lower()
    counter += 1
    #Checking Player's Sanity:
    if re.search(r'[бвгджзклмнпрстфхцчшщъы]{4}', word):
    ...
    if backskip(word):
        try:
            ...
            print('>{}(*~*):Да ты это слово сам выдумал!'.format(botname))
            continue
    #>>>Answering the User:
    try:
        if re.search(r'[^ ]+й', ' '.join(vocab[correspond[word[len(word)-1]]])):
            weapons = re.findall(r'[^ ]+й', ' '.join(vocab[correspond[word[len(word)-1]]]))
            reply = random.choice(weapons)
            used.append(reply)
